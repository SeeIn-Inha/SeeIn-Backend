...
def extract_receipt_info_with_gpt(ocr_text_content: str, openai_api_key: str) -> dict | None:
    ...
    - **결제 날짜 (transaction_date)**: 거래가 발생한 날짜 (YYYY년 MM월 DD일 형식)
    - **결제 시간 (transaction_time)**: 거래가 발생한 시간 (HH시 MM분 형식)
    - **총 결제 금액 (total_amount)**: 최종 결제된 금액 (숫자 뒤 '원' 붙이기)
    - **결제 항목 (items)**: 구매한 각 항목의 목록. 각 항목은 다음을 포함합니다:
        - **이름 (name)**: 상품 또는 서비스의 이름
        - **가격 (price)**: 해당 항목의 단가 또는 총 가격 (숫자 뒤 '원' 붙이기)
        - **수량 (quantity)**: 해당 항목의 수량 (숫자 뒤 '원' 붙이기). 수량이 불분명하면 1로 가정.
    ...
    {{
        ...
        "총 결재 금액": 25,500원,
        "결제 항목": [
            {{"상품명": "바나나", "가격": 3,000원, "수량": 2}},
            {{"상품명": "사과", "가격": 5,000원, "수량": 1}}
        ],
        "결제 수단": "신용카드"
    }}
